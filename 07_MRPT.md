...
具体的には、第二項の値が小さくなりすぎる際に数値的に不安定となるため、第二項の値を大きくするようにシフト（）をしてやるということです。
...
我々が「CASPT2による（摂動）エネルギー」というのは、この`E2 (Variational)`を用いて計算した値を採用します。
...
```
を作ってやり、このハミルトニアンを対角化することによりエネルギー（および一次波動関数）を計算しようというのがMS-CASPT2です。
...
- 参照空間の非対角項を無視(MS-CASPT2)するか、対角化をして直交させる(XMS-CASPT2)
...
ただし、`&RASSCF`の段階で`CIROOT`が適切に設定され、SA-CASSCFができていなければいけません。
```
&CASPT2
  IPEA = 0.00
  IMAGinary = 0.20
...
--
```
`Total CASPT2 energies`というのは、それぞれの状態ごとにCASPT2計算を行っただけの値です（すなわちSS-CASPT2）。
そして、`Effective Hamiltonian matrix`が有効ハミルトニアンを示しています。
...
MS-CASPT2のエネルギーは、`MS-CASPT2 Root  X     Total energy`のところからとってくることとなります。
...
- （うまい計算条件が分からないので保留）LiFのS<sub>0</sub>とS<sub>1</sub>のポテンシャルエネルギーカーブ（3から7 Angstromまで）を計算してみましょう。対称性はナシ、基底関数は6-31G(d)、活性空間はCAS(6e,6o)で、SCFは三状態の平均をとるようにしましょう。SS-CASPT2とXMS-CASPT2で比較をして、どちらがそれっぽいか考えましょう。この分子の自由度を踏まえると、二つの状態のポテンシャルエネルギーカーブはどのような振る舞いをするのが自然でしょうか。
